...
이 페이지 구조는 발표자료 흐름이 아니라 GitHub Pages로 제출할 독립형 시각화 결과물 탐색 구조다. 사용자는 전체 현황, 구 상세, 카테고리, 벤치마킹, 방법론을 자유롭게 이동하며 결과를 확인한다.
...
- 시설 필터 선택 시 표시 시설 변경
- 벤치마킹 버튼 클릭 시 선택 구가 유지된 상태로 `BenchmarkPage` 이동
...
- 선택 구의 약점 카테고리를 확인하고, 유사 조건의 상위 구를 비교 대상으로 제시한다.
...
- 구 선택 시 추천 결과와 비교 차트 갱신
- 추천 구 클릭 시 해당 구 상세 페이지로 이동
- `/benchmark?district=:districtId` query가 있으면 해당 구를 기본 선택
- 정렬 옵션 변경 시 구 선택 목록 순서 변경
...
- `district_scores.json`이 없으면 `구별 점수 데이터가 필요합니다.`를 표시한다.
- `benchmark_recommendations.json`이 없으면 `벤치마킹 추천 데이터가 필요합니다.`를 표시한다.
- 선택 구가 없으면 `분석할 구를 선택해 주세요.`를 표시한다.
- 선택 구의 점수가 없으면 `선택한 구의 점수 데이터가 없습니다.`를 표시한다.
- 선택 구의 추천 정보가 없으면 `이 구에 대한 벤치마킹 추천 데이터가 아직 생성되지 않았습니다.`를 표시한다.
- 추천 구의 점수 데이터가 없으면 `추천 구의 점수 데이터가 없습니다.`를 표시한다.

표현 원칙:

- `개선 여지가 있음`, `상대적으로 낮게 나타남`, `참고할 수 있음`처럼 비교 중심 표현을 사용한다.
- 특정 구를 부정적으로 단정하지 않는다.
- 추천 데이터가 없으면 브라우저에서 임의 추천을 생성하지 않는다.
